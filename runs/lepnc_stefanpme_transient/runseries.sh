...
executable_name="lepnc-stefanpme-transient"
...
do
  ...
  # Execute code
  $executable -m $meshfile --FinalTps $FinalTps --dt $timestep -b $bc -c $tcsol $tcdiff --testcaseNL $tcNL --powerPME $mPME --weight $weight --source $source --solver_type $solver_type
  r=$?
  if [ "$r" != "0" ]; then
    exit
  fi
