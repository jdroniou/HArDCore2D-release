PROGRAM compute_rates
  ...
  CHARACTER :: empty
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: meshsize, regmail, meshaniso, erl2p, erl2pml, erl2gradp, erlinfp, erflux, &
	...
	! On lit les donnees
  ALLOCATE(meshsize(nbtests),regmail(nbtests),meshaniso(nbtests),erl2p(nbtests),erl2pml(nbtests), &
      h1error(nbtests),energyerror(nbtests),nbedgedofs(nbtests))
	READ(10,*) empty
  DO i=1,nbtests
    READ(10,*) meshsize(i),erl2p(i),erl2pml(i),h1error(i),energyerror(i),nbedgedofs(i),regmail(i),meshaniso(i)
  ENDDO
  CLOSE(10)
	...
	print "(A)","-----------------------------------------\nL2 error ML : h, error, rate, (mesh reg)"
	CALL output_rates(nbtests,meshsize,regmail,meshaniso,erl2pml,20,"L2 error ML on $p$")
  ...
  DEALLOCATE(meshsize,regmail,meshaniso,erl2p,erl2pml,h1error,energyerror,nbedgedofs)
