...
executable_name="lepnc-stefanpme"
...
do
  ...
  # Execute code
  $executable -m $meshfile -b $bc -c $tcsol $tcdiff --testcaseNL $tcNL --powerPME $mPME --weight $weight --source $source --solver_type $solver_type
  r=$?
  if [ "$r" != "0" ]; then
    exit
  fi
...
cd $outdir
echo -e "meshsize L2error L2error_ml H1error EnergyError NbEdgeDOFs MeshReg MeshAnisotropy" > $errorsfile
for i in `seq 1 $nbmesh`; 
do
  meshsize=$(awk '/MeshSize:/ {print $NF}' results-$i.txt)
  L2error=$(awk '/L2error:/ {print $NF}' results-$i.txt)
  L2error_ml=$(awk '/L2error_ml:/ {print $NF}' results-$i.txt)
  ...
  \\\vspace{0.5cm}\\\\\\
    \\\begin{tikzpicture}
      \\\begin{loglogaxis}[ legend columns=-1, legend to name=legend ]
        ...
        \\\addplot table[x=meshsize,y=L2error_ml] {data_rates.dat};
