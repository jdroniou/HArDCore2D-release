...
do
    ...
    echo -e "------------------------------------------------------------------------------"
    # Execute code
    if($executable -m $meshfile.typ2 -k $k -s $solution -b $BC -x $stab_par -t $t -E $E -n $nu); then
      # Move outputs
...
echo -e "Deg MeshSize NbCells NbEdges DimEXCurl DimXGRad EnError Rate DisError Rate" > $outsubdir/$errorsfile
...
do
    ...
    DisError=$(awk '/DisError:/ {print $NF}' $outsubdir/results-$i.txt)
    ...
    if(($i \> 1)); then
      ...
      OldDisError=$(awk '/DisError:/ {print $NF}' $outsubdir/results-$imo.txt)
      DisErrorRate=$(perl -E "say sprintf(\"%.2f\", log($OldDisError/$DisError)/log($OldMeshSize/$MeshSize))")
      echo -e "$Degree $MeshSize $NbCells $NbEdges $DimEXCurl $DimXGrad $EnError $EnErrorRate $DisError $DisErrorRate" >> $outsubdir/$errorsfile
    else
  	  echo -e "$Degree $MeshSize $NbCells $NbEdges $DimEXCurl $DimXGrad $EnError -- $DisError --" >> $outsubdir/$errorsfile
    fi
done;
