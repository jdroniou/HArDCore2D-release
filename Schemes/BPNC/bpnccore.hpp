...
/*!	
* @defgroup BPNC 
* @brief Tools to implement the Bubble Polytopal Non-Conforming method
*/
...
/** The BPNCCore class provides basis functions for non-conforming schemes on generic polygonal meshes
*
* If using this code in a scientific publication, please cite the reference for the BPNC:
*
...
class BPNCCore : public HybridCore {

public:
	///@brief Class constructor: initialise the BPNCCore class, and creates non-conforming basis functions (and gradients)
  BPNCCore(
...
BPNCCore::BPNCCore(const Mesh* mesh_ptr,
					const size_t K,
					const size_t L,
          std::ostream & output):
...
//-----------------------------------------------------
std::vector<VectorRd> BPNCCore::create_ml_nodes(size_t iT) const {
...
std::tuple<std::vector<BPNCCore::cell_basis_type>,
			std::vector<BPNCCore::cell_gradient_type>,
			Eigen::MatrixXd>
	BPNCCore::create_nc_basis(size_t iT) const {
...
inline const BPNCCore::cell_basis_type& BPNCCore::nc_basis(size_t iT, size_t i) const {
  assert(iT < this->get_mesh()->n_cells());
  assert(i < _nc_bases[iT].size());
  return _nc_bases[iT][i];
}

inline const BPNCCore::cell_gradient_type& BPNCCore::nc_gradient(size_t iT, size_t i) const {
  assert(iT < this->get_mesh()->n_cells());
  assert(i < _nc_gradients[iT].size());
  return _nc_gradients[iT][i];
}

inline const VectorRd BPNCCore::ml_node(size_t iT, size_t i) const {
  assert(iT < this->get_mesh()->n_cells());
...
const std::vector<Eigen::ArrayXd> BPNCCore::nc_basis_quad(const size_t iT, const QuadratureRule quad) const {
...
const std::vector<Eigen::ArrayXXd> BPNCCore::grad_nc_basis_quad(const size_t iT, const QuadratureRule quad) const {
...
//-----------------------------------------------------
  ...
  /// Create the matrix of L2 products of two families (f_i) and (g_j) of functions
  /// (this is not really a Gram matrix, unless the two families are the same)
  Eigen::MatrixXd BPNCCore::gram_matrix(const std::vector<Eigen::ArrayXd>& f_quad, const std::vector<Eigen::ArrayXd>& g_quad, const size_t& nrows, const size_t& ncols, const QuadratureRule& quad, const bool& sym, std::vector<double> L2weight) const {
  ...
  /// Overloaded version of the previous one for vector-valued functions: the functions (F_i) and (G_j) are vector-valued functions
  Eigen::MatrixXd BPNCCore::gram_matrix(const std::vector<Eigen::ArrayXXd>& F_quad, const std::vector<Eigen::ArrayXXd>& G_quad, const size_t& nrows, const size_t& ncols, const QuadratureRule& quad, const bool& sym, std::vector<Eigen::Matrix2d> L2Weight) const {
...
template<typename Function>
Eigen::VectorXd BPNCCore::nc_interpolate_moments(const Function& f, size_t doe) const {
...
template<typename Function>
Eigen::VectorXd BPNCCore::nc_interpolate_ml(const Function& f, size_t doe) const {
	const Mesh* mesh_ptr = get_mesh();
	size_t ncells = mesh_ptr->n_cells();
	size_t nedges = mesh_ptr->n_edges();
  Eigen::VectorXd Xh = Eigen::VectorXd::Zero(DimPoly<Cell>(1)*ncells + nedges);
...
Eigen::VectorXd BPNCCore::nc_restr(const Eigen::VectorXd &Xh, size_t iT) const {
...
double BPNCCore::nc_L2norm(const Eigen::VectorXd &Xh) const {
  double value = 0.0;
...
double BPNCCore::nc_L2norm_ml(const Eigen::VectorXd &Xh) const {
  double value = 0.0;
...
double BPNCCore::nc_H1norm(const Eigen::VectorXd &Xh) const {
  double value = 0.0;
...
double BPNCCore::nc_evaluate_in_cell(const Eigen::VectorXd Xh, size_t iT, double x, double y) const {
  double value = 0.0;
	const Mesh* mesh_ptr = get_mesh();
  for (size_t i = 0; i < DimPoly<Cell>(1); i++) {
    const auto &phi_i = nc_basis(iT, i);
...
Eigen::VectorXd BPNCCore::nc_VertexValues(const Eigen::VectorXd Xh, const double weight) {
	const Mesh* mesh_ptr = get_mesh();
...
// Sign function
double BPNCCore::sign(const double s) const {
	double val = 0;
  if (s>0){
		val = 1;
	}else if (s<0){
