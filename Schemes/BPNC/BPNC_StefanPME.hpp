...
#ifndef _BPNC_STEFANPME_HPP
#define _BPNC_STEFANPME_HPP
...
//                            Class definition
// ----------------------------------------------------------------------------
/* The BPNC_StefanPME class provides an implementation of the BPNC method for the stationnary Stefan and PME problems.
...
public:
  ...
  BPNC_StefanPME(
		BPNCCore &nc,										///< instance of the core nonconforming class (with basis functions, etc.)
...
BPNC_StefanPME::BPNC_StefanPME(BPNCCore &nc, tensor_function_type kappa, size_t deg_kappa, source_function_type source, BoundaryConditions BC, solution_function_type exact_solution, grad_function_type grad_exact_solution, TestCaseNonLinearity::nonlinearity_function_type zeta, double weight, std::string solver_type, std::ostream & output)
  : nc(nc),
		kappa(kappa),
...
// ASSEMBLE AND SOLVE THE SCHEME
Eigen::VectorXd BPNC_StefanPME::solve() {
...
Eigen::VectorXd BPNC_StefanPME::apply_nonlinearity(const Eigen::VectorXd& Y, const std::string type) const {
...
Eigen::MatrixXd BPNC_StefanPME::diffusion_operator(const size_t iT) const {
...
Eigen::VectorXd BPNC_StefanPME::load_operator(const size_t iT) const {
...
Eigen::VectorXd BPNC_StefanPME::load_operator_ml(const size_t iT) const {
...
Eigen::VectorXd BPNC_StefanPME::residual_scheme(const Eigen::VectorXd& Xh) const{
...
double BPNC_StefanPME::L2_MassLumped(const Eigen::VectorXd Xh) const {
  const Mesh* mesh = nc.get_mesh();
	double value = 0.0;
...
double BPNC_StefanPME::EnergyNorm(const Eigen::VectorXd Xh) const {
  const Mesh* mesh = nc.get_mesh();
	double value = 0.0;
...
double BPNC_StefanPME::get_assembly_time() const {
  return double(_assembly_time) * pow(10, -9);
}

double BPNC_StefanPME::get_solving_time() const {
  return double(_solving_time) * pow(10, -9);
}

double BPNC_StefanPME::get_itime(size_t idx) const {
  return double(_itime[idx]) * pow(10, -9);
}

double BPNC_StefanPME::get_solving_error() const {
