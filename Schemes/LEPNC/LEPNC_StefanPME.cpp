...
//	} 
  ...
  // Diffusion tensor
  LEPNC_StefanPME::tensor_function_type kappa = [&](double x, double y, Cell* cell) {
			return tcase.diff(x,y,cell);
  };
	size_t deg_kappa = tcase.get_deg_diff();

  // Exact solution and gradient
  LEPNC_StefanPME::solution_function_type exact_solution = [&](double x, double y) {
			return tcase.sol(x,y);
  };
  LEPNC_StefanPME::grad_function_type grad_exact_solution = [&](double x, double y, Cell* cell) {
			return tcase.grad_sol(x,y,cell);
  };
	
  // Source term, adjusted for reaction term
  LEPNC_StefanPME::source_function_type source_term = [&](double x, double y, Cell* cell) {
  ...
  // Create the model equation
  LEPNC_StefanPME StefanPME_model(nc, kappa, deg_kappa, source_term, BC, exact_solution, grad_exact_solution, zeta, weight, solver_type);
