// Implementation of the HMM scheme on generic polygonal meshes for the Stefan and PME models
//
...
int main(int argc, const char* argv[]) {
  ...
  HybridCore hmm(mesh_ptr.get(), 0, 0, use_threads, output, false);
  ...
  // Initial solution
  std::function<double(VectorRd)> initial_condition = [&](VectorRd x)->double{
      return exact_solution(0.0, x);
    };
  UVector Xn = hmm.interpolate(initial_condition, 0, 0, 3);
  double ave_newton = 0;
  for (size_t idt = 1; idt <= NbTimeSteps; idt++){
    output << "Time step " << idt << "/" << NbTimeSteps << std::endl;
    UVector Xnp1 = StefanPME_model.iterate(dt*idt, dt, Xn);
    Xn.asVectorXd() = Xnp1.asVectorXd();
  ...
  output << "Computing error..." << std::endl;
	...
	UVector zetau = StefanPME_model.apply_nonlinearity(Xn, "fct");

  // Interpolate the exact solution
  //timer.start();
  std::function<double(VectorRd)> solution_FinalTps = [&](VectorRd x)->double{
    return exact_solution(FinalTps, x);
  };
  UVector Uh = hmm.interpolate(solution_FinalTps, 0, 0, 3);
	std::function<double(VectorRd)> exact_zetau = [&zeta, &solution_FinalTps](VectorRd x)->double {
			return zeta(solution_FinalTps(x), "fct");
		};
  UVector ZetaUh = hmm.interpolate(exact_zetau, 0, 0, 3);
  ...
  //timer.start();
  double H1error = hmm.H1norm(zetau - ZetaUh)/hmm.H1norm(ZetaUh);
